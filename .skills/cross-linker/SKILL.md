...
2. Read `$OBSIDIAN_WIKI_PATH/Wiki/index.md` to get the full inventory of pages and their one-line descriptions
3. Skim `$OBSIDIAN_WIKI_PATH/Wiki/log.md` to see what was recently ingested (focus linking effort on new pages)

## Step 1: Build the Page Registry

Glob all `.md` files in the vault (excluding `$OBSIDIAN_WIKI_PATH/Archives/`, `.obsidian/`). For each page, extract:
...
3. **Search for unlinked mentions** — check if the page's text contains any of these, without being wrapped in `[[...]]`:
   - Page filenames (e.g., the word "MyProject" appears but `[[$OBSIDIAN_WIKI_PATH/Wiki/projects/my-project/my-project]]` is missing)
...
- **Diacritic-insensitive matching** — normalize both the page name and the body text with Unicode NFKD (decompose accented characters to base + combining marks, strip combining marks) before comparing. This ensures body text "Muller" matches page `[[$OBSIDIAN_WIKI_PATH/Wiki/entities/müller]]` and vice versa.
...
| **Same project, no link** | +2 | Both under `$OBSIDIAN_WIKI_PATH/Wiki/projects/my-project/` but don't reference each other |
| **Mentioned entity/concept** | +2 | Page mentions "knowledge graphs" → link to `[[$OBSIDIAN_WIKI_PATH/Wiki/concepts/knowledge-graphs]]` |
| **Cross-category connection** | +2 | Source is in `$OBSIDIAN_WIKI_PATH/Wiki/concepts/`, target is in `$OBSIDIAN_WIKI_PATH/Wiki/entities/` (or `$OBSIDIAN_WIKI_PATH/Wiki/skills/` ↔ `$OBSIDIAN_WIKI_PATH/Wiki/synthesis/`) — different knowledge layers make this link more architecturally valuable |
...
**After:**
```markdown
This project uses [[$OBSIDIAN_WIKI_PATH/Wiki/concepts/knowledge-graphs|knowledge graphs]] to connect entities.
```
...
- [[$OBSIDIAN_WIKI_PATH/Wiki/projects/my-project/my-project]] — Also uses AI agents for research automation
- [[$OBSIDIAN_WIKI_PATH/Wiki/concepts/knowledge-graphs]] — Core technique used in this project
```
...
After the main linking pass, update affinity scores for all pages in `$OBSIDIAN_WIKI_PATH/Raw/Notes/` (pages with `promotion_status: misc` in their frontmatter, or located under the `$OBSIDIAN_WIKI_PATH/Raw/Notes/` directory).

For each misc page:

1. **Collect outgoing links** — all `[[wikilinks]]` in the page body
2. **Collect incoming links** — grep the vault for `[[$OBSIDIAN_WIKI_PATH/Raw/Notes/<slug>]]` and `[[<slug>]]` references
3. For each linked page (both directions), check if it belongs to a project:
   - Lives under `$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/`
...
|---|---|---|---|
| `$OBSIDIAN_WIKI_PATH/Wiki/projects/my-project/my-project.md` | 3 | EXTRACTED | 2 inline, 1 related |
| `$OBSIDIAN_WIKI_PATH/Wiki/entities/jane-doe.md` | 5 | INFERRED | 3 inline, 2 related |
| ... | | | |

### Orphan Pages Remaining: 2
- `$OBSIDIAN_WIKI_PATH/Wiki/references/foo.md` — no incoming or outgoing links found
- `$OBSIDIAN_WIKI_PATH/Wiki/concepts/bar.md` — could not find related pages

### Misc Promotion Candidates: N
Pages in $OBSIDIAN_WIKI_PATH/Raw/Notes/ that have ≥ 3 connections to a single project — ready to be promoted:

| Page | Top Project | Score |
|---|---|---|
| `$OBSIDIAN_WIKI_PATH/Raw/Notes/web-martinfowler-articles-microservices.md` | `Z01 - AI Obsidian` | 4 |

To promote: move the page to `$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/$OBSIDIAN_WIKI_PATH/Wiki/references/` and update all backlinks.

### Pages Skipped: 3
- `index.md`, `log.md` — special files
- `$OBSIDIAN_WIKI_PATH/Archives/*` — archived content
```
...
**`hot.md`** — Read `$OBSIDIAN_WIKI_PATH/hot.md` (create from the template in `wiki-ingest` if missing). Update **Recent Activity** with a one-line summary of what was linked — e.g. "Cross-linked 23 mentions across 12 pages; 2 orphans remain." Keep the last 3 operations. Update `updated` timestamp.
...
- **Don't touch pages in `$OBSIDIAN_WIKI_PATH/Archives/`.** Those are frozen snapshots.
