...
Archives live at `$OBSIDIAN_WIKI_PATH/Archives/`. Each archive is a timestamped directory containing a full copy of the wiki state at that point.

```
$OBSIDIAN_WIKI_PATH/Wiki/
├── Archives/
...
1. Create archive directory: `$OBSIDIAN_WIKI_PATH/Archives/YYYY-MM-DDTHH-MM-SSZ/`
2. Copy all category directories, `index.md`, `log.md`, `.manifest.json`, and `$OBSIDIAN_WIKI_PATH/Wiki/projects/` into the archive
3. Write `archive-meta.json` with reason `"snapshot"`
4. Append to `log.md`:
   ```
   - [TIMESTAMP] ARCHIVE reason="snapshot" pages=87 destination="$OBSIDIAN_WIKI_PATH/Archives/2026-04-06T10-30-00Z"
   ```
...
Remove all content from the category directories (`$OBSIDIAN_WIKI_PATH/Wiki/concepts/`, `$OBSIDIAN_WIKI_PATH/Wiki/entities/`, `$OBSIDIAN_WIKI_PATH/Wiki/skills/`, etc.) and the `$OBSIDIAN_WIKI_PATH/Wiki/projects/` directory. Keep:
- `$OBSIDIAN_WIKI_PATH/Archives/` (obviously)
...
2. — to reprocess OpenCode history
3. — to reprocess Kiro session history
...
Append to `log.md`:
```
- [TIMESTAMP] REBUILD archived_to="$OBSIDIAN_WIKI_PATH/Archives/2026-04-06T10-30-00Z" previous_pages=87
```
...
Read `$OBSIDIAN_WIKI_PATH/Archives/` directory. For each archive, read `archive-meta.json` and present:
...
4. Append to `log.md`:
   ```
   - [TIMESTAMP] RESTORE from="$OBSIDIAN_WIKI_PATH/Archives/2026-03-15T08-00-00Z" pages_restored=65
   ```
