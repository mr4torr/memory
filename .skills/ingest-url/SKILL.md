...
description: >
  ...
  if needed). Otherwise it goes to Raw/Bookmarks/ and gains project affinity over time. Use this skill
...
You are fetching a web page and distilling its content into an Z01 - AI Obsidian page. Where the page lands depends on whether you can detect a current project — if yes, it goes straight into that project's folder; if not, it goes to `$OBSIDIAN_WIKI_PATH/Raw/Bookmarks/` and is promoted later based on connection affinity.
...
1. Read `.env` (preferred) or `.env` (fallback) to get `OBSIDIAN_WIKI_PATH`
...
4. **No project context** — if the current directory IS the Z01 - AI Obsidian repo itself, or if detection produces a name that matches the wiki vault directory, treat it as "no project context" and fall back to `$OBSIDIAN_WIKI_PATH/Raw/Bookmarks/`.

**Normalise the project name:** lowercase, replace spaces and underscores with `-`, strip leading dots.

Once you have a candidate name, check whether `$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/` exists:
...
| No project context | Fall back to `$OBSIDIAN_WIKI_PATH/Raw/Bookmarks/` (Step 3c) |
...
- If in project mode: grep `$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/` for the URL string
- If in misc mode: grep `$OBSIDIAN_WIKI_PATH/Raw/Bookmarks/` for the URL string
...
Target: `$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/references/<slug>.md`

Create `$OBSIDIAN_WIKI_PATH/Wiki/references/` inside the project folder if it doesn't exist yet. This is a reference page, not a synthesis or concept page — it documents an external source that's relevant to the project.
...
```
$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/
...
Then add the page to: `$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/references/<slug>.md`

Report to the user: "Created new project `<project-name>` in the vault."

### Step 3c: No project context (misc fallback)

Target: `$OBSIDIAN_WIKI_PATH/Raw/Bookmarks/<slug>.md`

Create the `$OBSIDIAN_WIKI_PATH/Raw/Bookmarks/` directory if it does not exist yet.
...
**Project mode** (`$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/references/<slug>.md`):
```yaml
---
title: "<page title>"
category: references
...
**Misc mode** (`$OBSIDIAN_WIKI_PATH/Raw/Bookmarks/<slug>.md`):
```yaml
---
title: "<page title>"
category: misc
...
- `## Concepts` — wikilinks to related concept pages (`[[$OBSIDIAN_WIKI_PATH/Wiki/concepts/...]]`); create minimal stubs for important ones that don't exist yet
- `## Entities` — wikilinks to entity pages (`[[$OBSIDIAN_WIKI_PATH/Wiki/entities/...]]`) for people, tools, orgs mentioned
- `## Open Questions` — questions the source raises (omit section if none)
- `## Related` — wikilinks to any existing wiki pages this connects to; in project mode, always include a link back to `[[$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/<project-name>]]`
...
1. Check if it lives under `$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/`
...
> ⚡ Strong affinity detected: this page has **3+ connections** to `<project-name>`. Run the `cross-linker` skill to recompute affinity and then consider promoting this page to `$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/references/`.

## Step 6: Update Project Overview (project mode only)

Skip this step if in misc mode.

Read the project overview at `$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/<project-name>.md`. If the overview is a stub or doesn't mention this reference yet, add the new page to a `## References` section:

```markdown
## References

- [[$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/references/<slug>]] — <one-line summary>
```
...
```json
{
  ...
  "pages_created": ["$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/references/<slug>.md"],
  "pages_updated": ["$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/<project-name>.md"]
}
```
...
Project mode:
```
- [TIMESTAMP] INGEST_URL url="<url>" page="$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/references/<slug>.md" project="<project-name>" mode=project
```

Misc mode:
```
- [TIMESTAMP] INGEST_URL url="<url>" page="$OBSIDIAN_WIKI_PATH/Raw/Bookmarks/<slug>.md" affinity={} promotion_status=misc mode=misc
```

## Step 8: Update hot.md

Read `$OBSIDIAN_WIKI_PATH/Wiki/hot.md` (create from the template in `wiki-ingest` if missing). Update **Recent Activity** with what was just ingested — keep the last 3 operations. Update **Key Takeaways** if the page introduced a concept worth flagging. Update `updated` timestamp.
