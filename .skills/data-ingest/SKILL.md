...
name: data-ingest
description: >
  Ingest any raw text data, conversation logs, chat exports, or unstructured documents into the Z01 - AI Obsidian.
  Use this skill when the user wants to process data that isn't standard documents or OpenCode history —
  things like LLM exports, Slack threads, Discord logs, meeting transcripts, journal entries, CSV data,
...
You are ingesting arbitrary text data into an Z01 - AI Obsidian. The source could be anything — conversation exports, log files, transcripts, data dumps. Your job is to figure out the format, extract knowledge, and distill it into wiki pages.
...
- Use correct category directories (`Z01 - AI Obsidian/Wiki/concepts/`, `Z01 - AI Obsidian/Wiki/entities/`, `Z01 - AI Obsidian/Wiki/skills/`, etc.)
...
**`hot.md`** — Read `$OBSIDIAN_WIKI_PATH/Wiki/hot.md` (create from the template in `wiki-ingest` if missing). Update **Recent Activity** with the most meaningful thing extracted from this data source — last 3 operations max. Update `updated` timestamp.
