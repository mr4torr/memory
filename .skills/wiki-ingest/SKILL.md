...
You are ingesting source documents into an Z01 - AI Obsidian. Your job is not to summarize — it is to **distill and integrate** knowledge across the entire wiki.

## Before You Start

1. Read `.env` (preferred) or `.env.example` (fallback) to get `OBSIDIAN_WIKI_PATH` and `OBSIDIAN_SOURCES_DIR`. Only read the specific variables you need — do not log, echo, or reference any other values from these files.
...
If the QMD results show that 3+ papers touch the same concept, that concept almost certainly warrants a global `$OBSIDIAN_WIKI_PATH/Wiki/concepts/` page.
...
- Does this page already exist? (Check `index.md` and use Glob to search `OBSIDIAN_WIKI_PATH`)
...
**`hot.md`** — Read `$OBSIDIAN_WIKI_PATH/Wiki/hot.md` (create from template below if missing). Rewrite the **Recent Activity** section to reflect what you just ingested — keep it to the last 3 operations max. Update **Key Takeaways** and **Active Threads** if the content materially shifted them. Update the `updated` timestamp.
