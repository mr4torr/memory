...
description: >
  ...
  results into the Z01 - AI Obsidian. Use this skill when the user says "/wiki-research [topic]",
...
You are running an autonomous research loop on a topic, synthesizing what you find, and filing the results into the Z01 - AI Obsidian as permanent knowledge.

## Before You Start

1. Read `.env` (preferred) or `.env` (fallback) to get `OBSIDIAN_WIKI_PATH`
2. Read `$OBSIDIAN_WIKI_PATH/Wiki/index.md` to understand what's already in the wiki — don't re-research things the wiki covers well
3. Read `$OBSIDIAN_WIKI_PATH/Wiki/hot.md` if it exists — it surfaces recent context
4. Check `$OBSIDIAN_WIKI_PATH/Wiki/references/research-config.md` if it exists — it may define source preferences, domains to skip, or confidence rules for this vault

Confirm the research topic with the user if it's ambiguous. Then proceed.

## Research Configuration (optional)

If `$OBSIDIAN_WIKI_PATH/Wiki/references/research-config.md` exists in the vault, read it and apply any rules it defines:
...
### 2. Z01 - AI Obsidian/Wiki/concepts/ — One page per substantive concept
...
### 3. Z01 - AI Obsidian/Wiki/entities/ — Tools, organizations, people
...
### 4. Z01 - AI Obsidian/Wiki/synthesis/Research: [Topic].md — Master synthesis
...
- [ ] Synthesis page exists at `$OBSIDIAN_WIKI_PATH/Wiki/synthesis/Research: [Topic].md`
