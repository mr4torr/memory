...
You are distilling knowledge from the current project into the user's Z01 - AI Obsidian. This skill works from any project directory, not just the Z01 - AI Obsidian repo.

## Before You Start

1. Read `.env` to get:
   - `OBSIDIAN_WIKI_PATH` — where the wiki lives
   - `OBSIDIAN_VAULT_PATH` — where the Z01 - AI Obsidian repo is cloned (for reading other skills if needed)
3. Read `$OBSIDIAN_WIKI_PATH/Wiki/.manifest.json` to check if this project has been synced before.
4. Read `$OBSIDIAN_WIKI_PATH/Wiki/index.md` to know what the wiki already contains.
...
- OpenCode memory files if they exist (`.opencode/` in the project)
...
Goes under `$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/`:
...
Read `$OBSIDIAN_WIKI_PATH/Wiki/hot.md` (create from the template in `wiki-ingest` if missing). Rewrite **Recent Activity** with what was just synced — last 3 operations max. Update **Active Threads** if this project is an ongoing focus. Update **Key Takeaways** with the most important architectural insight or decision surfaced during this sync. Update `updated` timestamp.

Write conceptually: "Synced Z01 - AI Obsidian — added wiki-capture and wiki-research skills, core new capabilities are autonomous web research and conversation capture."

## Tips

- **Be aggressive about merging.** If the project uses React Server Components, don't create a new page if `$OBSIDIAN_WIKI_PATH/Wiki/concepts/react-server-components.md` already exists. Update the existing one and add this project as a source.
- **Consult the tag taxonomy.** Read `$OBSIDIAN_WIKI_PATH/Wiki/_meta/taxonomy.md` if it exists, and use canonical tags.
