...
1. Read `.env`, or fall back to `.env` in this repo, to get `OBSIDIAN_WIKI_PATH` and `OBSIDIAN_VAULT_PATH`.
...
1. Glob `$OBSIDIAN_WIKI_PATH/**/*.md` excluding `$OBSIDIAN_WIKI_PATH/Archives/`, `$OBSIDIAN_WIKI_PATH/Raw/`, `.obsidian/`, `node_modules/`, `index.md`, `log.md`, `_insights.md`.
...
1. Read the existing `$OBSIDIAN_VAULT_PATH/.obsidian/graph.json`. If it doesn't exist, start from this minimal default:
...
Append to `$OBSIDIAN_WIKI_PATH/Wiki/log.md`:
