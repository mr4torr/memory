...
2. Read `$OBSIDIAN_WIKI_PATH/Wiki/index.md` to understand existing wiki content (avoid duplicates)
3. Read `$OBSIDIAN_WIKI_PATH/Wiki/hot.md` if it exists — it gives context on recent activity
...
|---|---|---|
| `synthesis` | Multi-step analysis or an answer to a specific question that required reasoning | `$OBSIDIAN_WIKI_PATH/Wiki/synthesis/` |
| `concept` | A definition, framework, or mental model (what a thing *is*) | `$OBSIDIAN_WIKI_PATH/Wiki/concepts/` |
| `source` | Summary of an external document, article, or resource discussed | `$OBSIDIAN_WIKI_PATH/Wiki/references/` |
| `decision` | A strategic, architectural, or design choice and its rationale | `$OBSIDIAN_WIKI_PATH/Wiki/synthesis/` |
| `session` | A complete discussion summary when the conversation spans multiple topics | `$OBSIDIAN_WIKI_PATH/Wiki/journal/` |

If the content clearly belongs to a specific project (detected from context or user mention), place it under `$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/<category>/` instead.
...
- Not: "The user asked about X and OpenCode explained that..."
- Yes: "X works by..."
...
```
Saved to: $OBSIDIAN_WIKI_PATH/Wiki/projects/<name>/synthesis/<slug>.md
Title: <Title>
Type: synthesis
```
