...
2. Read `$OBSIDIAN_WIKI_PATH/Wiki/_meta/taxonomy.md` — this is the canonical tag list
3. Read `index.md` to understand the wiki's scope

## The Taxonomy File

The canonical tag vocabulary lives at `$OBSIDIAN_WIKI_PATH/Wiki/_meta/taxonomy.md`. It defines:
...
```
Glob: $OBSIDIAN_WIKI_PATH/**/*.md (excluding $OBSIDIAN_WIKI_PATH/Archives/, $OBSIDIAN_WIKI_PATH/Raw/, $OBSIDIAN_WIKI_PATH/Wiki/_meta/)
...
| Page                   | Tag Count | Tags                 |
| ---------------------- | --------- | -------------------- |
| `$OBSIDIAN_WIKI_PATH/Wiki/entities/jane-doe.md` | 8         | ai, ml, founder, ... |
```
...
If new canonical tags were agreed upon, append them to `$OBSIDIAN_WIKI_PATH/Wiki/_meta/taxonomy.md` in the correct section.

## Mode 3: Tagging a New Page

When you're creating a wiki page and need to choose tags:

1. Read `$OBSIDIAN_WIKI_PATH/Wiki/_meta/taxonomy.md`
...
3. Add it to `$OBSIDIAN_WIKI_PATH/Wiki/_meta/taxonomy.md` with:
...
**`hot.md`** — Read `$OBSIDIAN_WIKI_PATH/Wiki/hot.md` (create from the template in `wiki-ingest` if missing). Update **Recent Activity** with a one-line summary — e.g. "Tag audit: normalized 14 tags across 28 pages; 2 new canonical tags added." Keep the last 3 operations. Update `updated` timestamp.
