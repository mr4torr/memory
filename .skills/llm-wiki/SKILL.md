...
1. Read `.env` (preferred) or `.env` (fallback) to get `OBSIDIAN_WIKI_PATH` and `OBSIDIAN_SOURCES_DIR`
...
The wiki lives at the path configured via `OBSIDIAN_WIKI_PATH` in `.env`.
...
|---|---|---|
| `$OBSIDIAN_WIKI_PATH/Wiki/concepts/` | Ideas, theories, mental models | `$OBSIDIAN_WIKI_PATH/Wiki/concepts/transformer-architecture.md` |
| `$OBSIDIAN_WIKI_PATH/Wiki/entities/` | People, orgs, tools, projects | `$OBSIDIAN_WIKI_PATH/Wiki/entities/andrej-karpathy.md` |
| `$OBSIDIAN_WIKI_PATH/Wiki/skills/` | How-to knowledge, procedures | `$OBSIDIAN_WIKI_PATH/Wiki/skills/fine-tuning-llms.md` |
| `$OBSIDIAN_WIKI_PATH/Wiki/references/` | Summaries of specific sources | `$OBSIDIAN_WIKI_PATH/Wiki/references/attention-is-all-you-need.md` |
| `$OBSIDIAN_WIKI_PATH/Wiki/synthesis/` | Cross-cutting analysis across sources | `$OBSIDIAN_WIKI_PATH/Wiki/synthesis/scaling-laws-debate.md` |
| `$OBSIDIAN_WIKI_PATH/Wiki/journal/` | Timestamped observations, session logs | `$OBSIDIAN_WIKI_PATH/Wiki/journal/2024-03-15.md` |

### Projects

Knowledge often belongs to a specific project. The `$OBSIDIAN_WIKI_PATH/Wiki/projects/` directory mirrors this:
...
│   └── side-project/
│       └── ...
├── Wiki/concepts/             ← global (cross-project) knowledge
├── Wiki/entities/
├── Wiki/skills/
└── ...
```

**When knowledge is project-specific** (a debugging technique that only applies to one codebase, a project-specific architecture decision), put it under `$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/<category>/`.
...
**Naming rule:** The project overview file must be named `<project-name>.md`, not `_project.md`. Obsidian's graph view uses the filename as the node label — `_project.md` makes every project appear as `_project` in the graph, making it unreadable. So `$OBSIDIAN_WIKI_PATH/Wiki/projects/my-project/my-project.md`, `$OBSIDIAN_WIKI_PATH/Wiki/projects/another-project/another-project.md`, etc.
...
category: project
tags: [ai, web, backend]
source_path: $OBSIDIAN_WIKI_PATH/Wiki/projects/Users-name-Documents-projects-my-project
...
## Key Concepts
- [[$OBSIDIAN_WIKI_PATH/Wiki/concepts/some-api]] — used for core functionality
- [[$OBSIDIAN_WIKI_PATH/Wiki/projects/my-project/$OBSIDIAN_WIKI_PATH/Wiki/concepts/main-architecture]] — project-specific architecture

## Related
- [[$OBSIDIAN_WIKI_PATH/Wiki/entities/some-service]] — deployment platform
```
...
- [2024-03-17T10:00:00Z] ARCHIVE reason="rebuild" pages=87 destination="$OBSIDIAN_WIKI_PATH/Archives/..."
- [2024-03-17T10:05:00Z] REBUILD archived_to="$OBSIDIAN_WIKI_PATH/Archives/..." previous_pages=87
```
...
- [[$OBSIDIAN_WIKI_PATH/Wiki/references/attention-is-all-you-need]] — Original paper
```
...
- `OBSIDIAN_WIKI_PATH` — Where the wiki lives **(required)**
...
| **Rebuild** | Major drift, fresh start needed | Archive current wiki to `$OBSIDIAN_WIKI_PATH/Archives/`, clear, reprocess all sources |
