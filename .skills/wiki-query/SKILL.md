...
1. Read `.env` to get `OBSIDIAN_WIKI_PATH` (works from any project). Fall back to `.env` if you're inside the Z01 - AI Obsidian repo.
2. If `$OBSIDIAN_WIKI_PATH/Wiki/hot.md` exists, read it first — it gives you instant context on recent activity. If the user's question is about something ingested recently, hot.md may answer it before you even open `index.md`.
3. Read `$OBSIDIAN_WIKI_PATH/Wiki/index.md` to understand the wiki's scope and structure
...
**Also search `papers` when the question may have source material in `$OBSIDIAN_WIKI_PATH/Wiki/Raw/`:**
