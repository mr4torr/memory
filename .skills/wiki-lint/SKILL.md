...
2. Read `Wiki/index.md` for the full page inventory
3. Read `Wiki/log.md` for recent activity context
...
- Pages with zero incoming links (except `Wiki/index.md` and `Wiki/log.md`) are orphans
...
Verify `$OBSIDIAN_WIKI_PATH/Wiki/index.md` matches the actual page inventory.

**How to check:**
- Compare pages listed in `$OBSIDIAN_WIKI_PATH/Wiki/index.md` to actual files on disk
- Check that summaries in `$OBSIDIAN_WIKI_PATH/Wiki/index.md` still match page content
...
- Apply these thresholds:
  - **AMBIGUOUS > 15%**: flag as "speculation-heavy" — even 1-in-7 claims being genuinely uncertain is a signal the page needs tighter sourcing or should be moved to `$OBSIDIAN_WIKI_PATH/Wiki/synthesis/`
...
**How to fix:**
- For ambiguous-heavy: re-ingest from sources, resolve the uncertain claims, or split speculative content into a `$OBSIDIAN_WIKI_PATH/Wiki/synthesis/` page
...
Find pages in `$OBSIDIAN_WIKI_PATH/Raw/Notes/` that have accumulated enough project affinity to be promoted.

**How to check:**
- Glob `$OBSIDIAN_WIKI_PATH/Raw/Notes/*.md`
...
- To promote: move the page to `$OBSIDIAN_WIKI_PATH/Wiki/projects/<project-name>/references/` (or another appropriate category), update its `category` frontmatter, remove `promotion_status`, and grep the vault for backlinks to update them

### 11. Synthesis Gaps

Identify high-value synthesis opportunities the wiki is missing — concept pairs that co-occur across many pages but have no `$OBSIDIAN_WIKI_PATH/Wiki/synthesis/` page connecting them.

**How to check:**
- List all pages in `$OBSIDIAN_WIKI_PATH/Wiki/synthesis/` — collect the concept pairs each one already covers (from its `[[wikilinks]]` or title)
- Pick 10-15 frequently linked concepts from `$OBSIDIAN_WIKI_PATH/Wiki/concepts/` and `$OBSIDIAN_WIKI_PATH/Wiki/entities/`
- For each pair, run a quick grep to count pages that link to both:
  ```bash
  grep -rl "\[\[ConceptA\]\]" "$OBSIDIAN_WIKI_PATH" --include="*.md" > /tmp/a.txt
  grep -rl "\[\[ConceptB\]\]" "$OBSIDIAN_WIKI_PATH" --include="*.md" > /tmp/b.txt
...
- `$OBSIDIAN_WIKI_PATH/Wiki/concepts/foo.md` — no incoming links

### Broken Wikilinks (N found)
- `$OBSIDIAN_WIKI_PATH/Wiki/entities/bar.md:15` — links to [[nonexistent-page]]

### Missing Frontmatter (N found)
- `$OBSIDIAN_WIKI_PATH/Wiki/skills/baz.md` — missing: tags, sources

### Stale Content (N found)
- `$OBSIDIAN_WIKI_PATH/Wiki/references/paper-x.md` — source modified 2024-03-10, page last updated 2024-01-05

### Contradictions (N found)
- `$OBSIDIAN_WIKI_PATH/Wiki/concepts/scaling.md` claims "X" but `$OBSIDIAN_WIKI_PATH/Wiki/synthesis/efficiency.md` claims "not X"

### Index Issues (N found)
- `$OBSIDIAN_WIKI_PATH/Wiki/concepts/new-page.md` exists on disk but not in index.md

### Missing Summary (N found — soft)
- `$OBSIDIAN_WIKI_PATH/Wiki/concepts/foo.md` — no `summary:` field
- `$OBSIDIAN_WIKI_PATH/Wiki/entities/bar.md` — summary exceeds 200 chars

### Provenance Issues (N found)
- `$OBSIDIAN_WIKI_PATH/Wiki/concepts/scaling.md` — AMBIGUOUS > 15%: 22% of claims are ambiguous (re-source or move to $OBSIDIAN_WIKI_PATH/Wiki/synthesis/)
- `$OBSIDIAN_WIKI_PATH/Wiki/entities/some-tool.md` — drift: frontmatter says inferred=0.10, recomputed=0.45
- `$OBSIDIAN_WIKI_PATH/Wiki/concepts/transformers.md` — hub page (31 incoming links) with INFERRED=28%: errors here propagate widely
- `$OBSIDIAN_WIKI_PATH/Wiki/synthesis/speculation.md` — unsourced synthesis: no `sources:` field, 55% inferred
...
- `$OBSIDIAN_WIKI_PATH/Wiki/entities/user-records.md` — contains `email:` value pattern but no `visibility/pii` tag
- `$OBSIDIAN_WIKI_PATH/Wiki/concepts/auth-flow.md` — tagged `visibility/pii` but missing `sources:` frontmatter
- `_meta/taxonomy.md` — contains `visibility/internal` entry (system tag must not be in taxonomy)

### Misc Promotion Candidates (N found)
Pages in $OBSIDIAN_WIKI_PATH/Raw/Notes/ that have ≥ 3 connections to a single project and are ready to be promoted:

| Page | Top Project | Affinity Score |
|---|---|---|
| `$OBSIDIAN_WIKI_PATH/Raw/Notes/web-martinfowler-articles-microservices.md` | `$OBSIDIAN_WIKI_PATH` | 4 |
